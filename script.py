...
class SheetStorage:
    ...
    def merge_data(self, sheet_name, new_data, config):
        ...
        for col in new_df.columns:
            if pd.api.types.is_datetime64_any_dtype(new_df[col]) or \
            isinstance(new_df[col].iloc[0], (datetime, date)):
                new_df[col] = pd.to_datetime(new_df[col]).dt.strftime('%Y-%m-%d %H:%M:%S')
                if not existing_data.empty and col in existing_data.columns:
                    existing_data[col] = pd.to_datetime(existing_data[col]).dt.strftime('%Y-%m-%d %H:%M:%S')

        if not existing_data.empty:
            # Update existing records and add new ones
            merged = pd.concat([existing_data, new_df]).drop_duplicates(subset=config['key_columns'], keep='last')
        else:
            merged = new_df
...
async def main():
    ...
    if cached_data:
        ...
        async with TelegramClient('anon', config.api_id, config.api_hash) as client:
            ...
            logger.debug(json.dumps(all_stats, default=datetime_handler, ensure_ascii=False, indent=2))
    ...
    for chat in all_stats['chats']:
        for topic_id, topic_data in chat['topics'].items():
            chat_topics.append({
                ...
                'hour': datetime.now(config.timezone).replace(minute=0, second=0, microsecond=0),

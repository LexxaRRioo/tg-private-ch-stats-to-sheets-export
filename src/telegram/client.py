...
async def get_messages_by_hour(client, chat, topic_id, timezone, start_date=None, end_date=None):
    messages_by_hour = {}
    total_messages = 0
    ...
    async for message in client.iter_messages(chat, reply_to=topic_id, reverse=True, filter=message_filter):
        total_messages += 1
    ...
    for retry in range(max_retries):
        try:
            ...
            async for message in client.iter_messages(channel, limit=100):
                if message.text:
                    messages.append({
                        "date": message.date.astimezone(timezone).strftime("%Y-%m-%dT%H:%M:%S"),
                        "text": message.text,
